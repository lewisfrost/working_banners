(function(window) {
	...
	_p.form = function(rad,time){
		var rad = rad || 20;
		var time = time || 0.5;
		var len = this._circlesArray.length;
		for (var i = 0; i < len; i++) {
			var progDelay = (i/len);
			var randomStart = new createjs.Point(this._positions[i].x+(Math.random()*rad*2)-(rad), this._positions[i].y+(Math.random()*rad*2)-(rad));
				TweenLite.fromTo(this._circlesArray[i], Math.random()+0.5*time, {scaleX:0, scaleY:0, alpha:0, x:randomStart.x, y:randomStart.y},{scaleX:1, scaleY:1, alpha:1, x:this._positions[i].x, y:this._positions[i].y, delay:Math.random()*progDelay*time, ease:Sine.easeOut});
			}
	}
	_p.moveIndividualParticle = function(id, targetId, time, delay){
		...
		TweenMax.to(particle, time, {x:target.x, y:target.y, ease:Power2.easeIn, delay:delay, repeat: -1, yoyo:true, repeatDelay:2});
	}
				...
				switch(style) {
					case "swirl":
						...
						if(this._surplusCircles.indexOf(this._circlesArray[k])<0) {
							TL.add([TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1.5*t), {x:swirlPos.x, y:swirlPos.y, delay:Math.random()*0.1, ease:Circ.easeOut}),
							TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1*t), {alpha:0.2, scaleX:0.1, scaleY:0.1, delay:Math.random()*0.1+(0.25*t), ease:Linear.easeNone})]);
							TL.add([TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1*t), {x:this._positions[k].x-20, y:this._positions[k].y, delay:Math.random()*0.2, ease:Sine.easeInOut}),
							TweenLite.to(this._circlesArray[k], Math.random()*0.25+(0.75*t), {alpha:1, scaleX:1, scaleY:1, delay:Math.random()*0.1, ease:Linear.easeNone})]);
						} else {
							if(k<len-1) {
								TL.add([TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1.5*t), {x:swirlPos.x, y:swirlPos.y, delay:Math.random()*0.1, ease:Circ.easeOut}),
								TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1*t), {alpha:0, scaleX:0.1, scaleY:0.1, delay:Math.random()*0.1+(0.25*t), ease:Linear.easeNone})]);
							} else {
								TL.add([TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1.5*t), {x:swirlPos.x, y:swirlPos.y, delay:Math.random()*0.1, ease:Circ.easeOut}),
								TweenLite.to(this._circlesArray[k], Math.random()*0.25+(1*t), {alpha:0, scaleX:0.1, scaleY:0.1, delay:Math.random()*0.1+(0.25*t), ease:Linear.easeNone, onComplete:this.removeSurplusParticles, onCompleteScope:this})]);
							}
						}
						break;
				}
	...
	_p.disperse = function(xOff, yOff, style, t, rad) {
		...
		this._isAnimating = true;
		var newPoint = new createjs.Point();
		for(var i = 0; i<this._circlesArray.length; i++) {
			var progDelay = i/this._circlesArray.length;
			var randomFactorX = Math.random()*rad*2;
			var randomFactorY = Math.random()*rad*2;
			newPoint.x = this._circlesArray[i].x +randomFactorX-rad;
			newPoint.y = this._circlesArray[i].y +randomFactorY-rad;
			if (style == "small") {
				TweenLite.to(this._circlesArray[i], t, {x:newPoint.x, y:newPoint.y, delay:t*Math.random()*0.3, ease:Sine.easeIn});
				TweenLite.to(this._circlesArray[i], t, {alpha: 0,  scaleX:0, scaleY:0, delay:t*Math.random()*0.3, ease:Linear.easeNone});
				//TODO - Clear all dispersed particles out to save memory...
			}
		}
