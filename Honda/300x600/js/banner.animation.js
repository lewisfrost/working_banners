...
Banner.prototype.createElements = function() {
  ...
  this.woman = this.smartObject({
    backgroundImage: 'images/woman.png',
    left: 'none',
    top: 'none',
    parent: this.banner
  });
...
Banner.prototype.setup = function() {
  ...
  this.bg.set({autoAlpha: 0, x: 8, scale: 0.9});

  this.woman.set({autoAlpha: 0, right: -30, bottom: -80, scale: 0.8});

  this.logo.centerHorizontal();
  this.logo.set({top: 20});

  this.copy.centerHorizontal();
  this.copy.set({top: 98});
  this.copyA.set({autoAlpha: 0});

  this.cta.centerHorizontal();
  this.cta.set({top: 435, autoAlpha: 0});
  this.ctaCopy.set({x: -14});
  this.ctaArrows.centerHorizontal();
  this.ctaArrows.set({x: 114, y: 24});
...
Banner.prototype.animate = function() {
  this.tl = new TimelineLite()
    ...
    .add([
      ...
      TweenLite.to(this.woman, 1, {autoAlpha: 1, delay: 1}),
      TweenLite.to(this.woman, 8, {y: 10, x: 10, scale: 1.1, ease: Quad.easeInOut}),
      TweenLite.to(this.bg, 5, {scale: 1, y: 20, ease: Quad.easeInOut}),
